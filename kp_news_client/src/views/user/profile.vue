<template>
  <div id="user-profile">
    <van-nav-bar title="个人资料" left-text="返回" left-arrow @click-left="$router.back()"></van-nav-bar>
    ...
    <van-form @submit="onSubmit">
      ...
      <van-field name="radio" label="性别">
        <template #input>
          <van-radio-group v-model="userForm.sex" direction="horizontal">
            <van-radio name="男" checked-color="red">男</van-radio>
            <van-radio name="女" checked-color="red">女</van-radio>
          </van-radio-group>
        </template>
      </van-field>
      <van-field
      ...
      <div style="margin: 16px;">
        <van-button round block type="danger" color="red" native-type="submit">提交</van-button>
      </div>
    </van-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async afterRead(file) {
      ...
      try {
        let result = await uploadAvatar(file, this.userForm.id);
        console.log(result.data.headImg);
        this.userForm.headImg = result.data.headImg;
      } catch (error) {
        console.log(error);
      }
